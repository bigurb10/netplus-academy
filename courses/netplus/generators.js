...
(function () {
  ...
  function subnetting() {
    ...
    if (kind <= 3) {
      ...
      if (kind === 0) {
        return build("u4l3", `What is the network address for the host ${addr}?`, netStr,
          [`${base}.${nextNet}`, `${base}.${prevNet}`, `${base}.${Math.max(0, net - 1)}`, `${base}.${bcast}`],
      ...
      if (kind === 1) {
        return build("u4l3", `What is the broadcast address of the subnet containing ${addr}?`, bcastStr,
          [`${base}.${Math.min(255, bcast + 1)}`, `${base}.${net}`, `${base}.${Math.min(255, bcast + block)}`, `${base}.255`],
      ...
      return build("u4l3", `What is the range of usable host addresses in the subnet containing ${addr}?`, `${base}.${net + 1} to ${base}.${bcast - 1}`,
        [`${base}.${net} to ${base}.${bcast}`, `${base}.${net + 1} to ${base}.${bcast}`, `${base}.${net} to ${base}.${bcast - 1}`, `${base}.${Math.max(0, prevNet + 1)} to ${base}.${Math.max(0, net - 2)}`],
    ...
    return build("u4l3", `What is the network address for the host ${b1}.${b2}.${third}.${fourth}/${prefix}?`, `${b1}.${b2}.${net}.0`,
      [`${b1}.${b2}.${third}.0`, `${b1}.${b2}.${Math.min(255, net + block)}.0`, `${b1}.${b2}.${bcast3}.0`, `${b1}.${b2}.0.0`],
