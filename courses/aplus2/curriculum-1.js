...
The exam calls these vendor life-cycle limitations: an operating system past end-of-life, or one that no longer receives updates on that hardware, is a limitation the technician must plan around.
...
Compatibility concerns between operating systems come up whenever files, applications, or drives move between Windows, macOS, and Linux.
...
Storage requirements mean free disk space for the installer, the installed program, and its data, checked against the drive before installing.
...
Application to OS compatibility is the first check: 32-bit or 64-bit, the minimum OS version, and whether the application was built for this operating system at all.
...
The suite typically includes email, file storage and synchronization, word processing, spreadsheet, and presentation tools, plus collaboration and identity synchronization with the directory.
...
Any command followed by /? (for example, ipconfig /?) prints its help and switches; the exam writes this as [command name] /?.
...
Its General options cover opening folders in the same window, single-click or double-click, and privacy; its View options cover how to view hidden files, file extensions, and protected operating system files.
...
File Explorer navigation to network paths means typing \\\\server\\share in the address bar to browse a share without mapping it.
...
This is the client's Internet Protocol (IP) addressing scheme: static or dynamic, with the address, subnet mask, gateway, and DNS servers.
...
The exam groups these as establish network connections: VPN, wireless (WLAN), wired, and wireless wide area network (WWAN) cellular.
...
The uninstallation process is dragging the application from Applications to the Trash, or running the vendor's uninstaller when it installed system components.
